...
from hummingbot.connector.exchange.bison import (
    ...
    bison_eip712_structs as bison_eip712_structs,
...
class BisonExchange(ExchangePyBase):
    web_utils = web_utils
    ...
    @staticmethod
    def bison_order_type(order_type: OrderType) -> str:
        return order_type.name.upper()

    @staticmethod
    def to_hb_order_type(bison_type: str) -> OrderType:
        return OrderType[bison_type]
    ...
    def _create_order_book_data_source(self) -> OrderBookTrackerDataSource:
        return BisonAPIOrderBookDataSource(
    ...
    def _create_user_stream_data_source(self) -> UserStreamTrackerDataSource:
        return BisonAPIUserStreamDataSource(
            auth=self._auth,
            trading_pairs=self._trading_pairs,
            api_factory=self._web_assistants_factory,
            connector=self,
    ...
    async def _place_order(
        ...
        **kwargs,
        ...
        order = bison_eip712_structs.Order(
    ...
    async def _place_cancel(self, order_id: str, tracked_order: InFlightOrder):
        ...
        cancel = bison_eip712_structs.Cancellation(
    ...
    async def _request_order_status(self, tracked_order: InFlightOrder) -> OrderUpdate:
        ...
        try:
            ...
            self.logger().warning(f"Error requesting orders from Bison sequencer: {e}")
        ...
        if updated_order_data.get("status") == "failure":
            ...
            try:
                ...
                self.logger().warning(f"Error requesting orders from Bison indexer: {e}")
    ...
    async def _get_account(self):
        ...
        if response is None or "failure" in response["status"] or "data" not in response:
            if "error_code" in response and response["error_code"] in CONSTANTS.ERRORS:
                raise IOError(f"IP address issue from Bison {response}")
    ...
    async def _get_symbols(self):
        ...
        if response is None or "status" in response:
            raise IOError("Unable to get Bison symbols")
    ...
    async def _get_contracts(self):
        ...
        if response is None or "failure" in response["status"] or "data" not in response:
            raise IOError("Unable to get Bison contracts")
